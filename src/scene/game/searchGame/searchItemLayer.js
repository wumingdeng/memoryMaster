...
var searchItemLayer = cc.Layer.extend({
    ...
    init:function(game) {
        ...
        this.scenePath = "res/gameScene/searchGame/gameScene_" + game.sceneID;

        var sceneFileName = this.scenePath + "/gameScene_" + game.sceneID + ".json";
        ...
        var touchsNum = 0; //触点数;
        ///添加场景可缩放;
        //this.area = new cc.ScrollView();
        ////    this.area.setPosition(vsize.width/2,vsize.height/2);
        //this.area.setTouchEnabled(true);
        //this.area.setContentSize(vsize);
        //this.area.setDirection(cc.SCROLLVIEW_DIRECTION_BOTH);
        //this.area.setViewSize(vsize);
        //this.area.setContainer(this._itemScene)
        ////this.area.setMinScale(1);
        //if (game.gameType == 2) {  //黑夜模式不可缩放移动;
        //    this.area.setTouchEnabled(false);
        //} else {
        //    this.area.setMaxScale(2);
        //}
        //
        //this.area.setBounceable(true);
        //this.area.setClippingToBounds(true);
        //var lastZoom = 1;  //初始的缩放值;
        //var delegate = {};
        //delegate.scrollViewDidScroll = function (a,b) {
        //    //边界判断;
        //    var leftBorder = vsize.width/2 / this._itemScene.width/2;
        //    leftBorder = Math.max(0,leftBorder); //如果小于0就取0;
        //    if (this._itemScene.getPositionX() > leftBorder) {
        //        this._itemScene.setPositionX(leftBorder);
        //    }
        //    if (this._itemScene.getPositionY() > 0) {
        //        this._itemScene.setPositionY(0);
        //    }
        //    var rightBorder = vsize.width / this._itemScene.width;
        //    if (rightBorder > 0) {
        //        rightBorder = rightBorder/2;
        //    }
        //    if (this._itemScene.getPositionX() < rightBorder) {
        //        this._itemScene.setPositionX(rightBorder);
        //    }
        //    if (this._itemScene.getPositionY() < vsize.height / this._itemScene.getBoundingBox().height) {
        //        this._itemScene.setPositionY(vsize.height / this._itemScene.getBoundingBox().height);
        //    }
        //    if (this.compass) {
        //        //更新目标物体的坐标 并且让罗盘始终指向目标;
        //        //this.compass.setTarget(cfun.getWorldPosition(this.compassTarget));
        //        //this.compass.pointToTarget();
        //    }
        //};
        //
        //
        //this.area.setDelegate(delegate);
        ////黑夜模式不缩放;
        ////if (game.gameType != 2) {
        ////    this.area.registerScriptHandler(scrollView1DidScroll,cc.SCROLLVIEW_SCRIPT_SCROLL);
        ////    this.area.registerScriptHandler(scrollView1DidZoom,cc.SCROLLVIEW_SCRIPT_ZOOM);
        ////}
        //this._contrlLayer.addChild(this.area);
        var progressTime = 0;
    ...
    //        }
            ...
            //if (this._dragItem) {
            //    //正在拖动 放弃点击判断;
            //    this.area.setTouchEnabled(true);  //设置成能滑动场景;
            //    //判断物品是否移到了目标点;
            //    var target = this.itemPanel.getChildByTag(this._dragItem.target);
            //    var rect = target.getBoundingBox();
            //    if (rect.width == 0) {     //如果是0 说明是动画节点;
            //        var img = target.getChildren()[1];
            //        rect.width = cfun.getWidth(img);
            //        rect.height = cfun.getHeight(img);
            //        rect.x = rect.x / rect.width/2;
            //        rect.y = rect.y / rect.height/2;
            //    }
            //    var p = cc.p(this._dragItem.getPosition());
            //    if (cc.rectContainsPoint(rect,p)) {
            //        cc.log("nice drag");
            //        //table.removeItem(this._actionItems,this._dragItem.getTag());
            //        this._dragItem.removeFromParent();
            //        this._dragItem = null;
            //        target.callback = function();
            //            if (target.target) {
            //                var nextItem = searchItemLayer.itemPanel.getChildByTag(target.target);
            //                //组合物品播完后改变遮挡物品的状态;
            //                nextItem.playIt();
            //            }
            //        }
            ...
            //        location = searchItemLayer.itemPanel.convertToNodeSpace(location);
            //        this._dragItem.backout(location);
            //    }
            //    return;
            //}
            ...
            if (!hasCurListItem) {
                if (dispatcherTime<9) {
                    ...
                    //var event = cc.EventCustom:new(SEARCH_CLICK_ERROR_EVENT);    //发事件;
        ...
        function onTouchBegan(touch, event){
            ...
            location = cc.pAdd(location,clickArea[dispatcherTime]);
            var p = self._itemScene.convertToNodeSpace(location);
            //var _,item = getClickItem(p,this._actionItems);
            //if (item) {
            //    this.deleteAllTip();
            //    if (item.actionType == 3) {    //可拖动的物品;
            //        if (not item.isLock) {
            //            this._dragItem = item;
            //            this._dragItem.setLocalZOrder(10000);
            //            this.area.setZoomScale(1,true);   //缩放还原;
            //            this.area.setTouchEnabled(false);  //设置成不能能滑动场景;
            //            var up = cc.MoveTo:create(0.2,cc.p(p.x,p.y + 70));
            //            var bigger = cc.scaleTo:create(0.2,1.6);
            //            var smaller = cc.scaleTo:create(0.3,1);
            //            var action = cc.sequence:create(cc.Spawn:create(up,bigger),smaller);
            ...
            //        cc.log(item.tipText);
            //        //播放提示效果;
            //        this.actionItemTextTip(item);
            //        touchsNum = touchsNum / 1;   //不触发touchend 所以触点减1;
            //        return false;
            //    } } } else { { if ( item.actionType == 5) {
            //        return true;     //跳过touchBegan 在touchEnd中处理;
            //    } } else { {
            //        touchsNum = touchsNum / 1;   //不触发touchend 所以触点减1;
            //        return false;    //屏蔽触摸;
            //    }
    ...
    //        this.deleteTipByTag();
        ...
        for (var key in this.itemPanel.getChildren()) {
            ...
            //var tag = item.getTag();
            //if (tag >= 1 && tag <= Iteminfo.data.length) {
            //    if (CaseHubData.Iteminfo.data[tag][CaseHubData.Iteminfo.ISEVIDENCE] == 2) {
            //
            //    } } else { if ( CaseHubData.Iteminfo.data[tag][CaseHubData.Iteminfo.ISPRIZE] == 1) {
            //        if (not gfun.checkHasItemByTable(searchScene.prizeItemData,tag,CaseHubData.Iteminfo.ID)) {
            //            searchItemLayer.itemPanel.removeChildByTag(tag);
            //            cc.log("removeChildByTag="..tag.." because the item is prize");
            //        }
            //    }  } else { if ( removeItemWithSameName(tag)) {
            //        searchItemLayer.itemPanel.removeChildByTag(tag);
            //    }
            //
            //}
    ...
    //                targetP = cfun.getWorldSpaceLocation(item);
                    ...
                    if (i == num) {
                        //发事件 现在是告诉黑夜模式的遮罩该移动到提示处。。;
                        //var tipEvent = cc.EventCustom:new(SEARCH_CLICK_TIP);    //发事件;
                        //tipEvent.tipX = targetP.x;
                        //tipEvent.tipY = targetP.y;
                        //cc.Director.getInstance().getEventDispatcher():dispatchEvent(tipEvent);
                    }
                }
                if (isSkill) {
                    runActionTip();
        ...
        //if (this.area.getZoomScale() == 1) {
        //    runActionOfTipEffect();
        //} else {
        //    function changeZoom(target,event) {
        //        if (event == ccui.ScrollView.EVENT_SCROLLING) {
        //            if (this.area.getZoomScale() == 1) {
        //                this.area._scrollViewEventListener = null;
        //                runActionOfTipEffect();
        //            }
        //        }
        //    }
        //    this.area.addEventListener(this,changeZoom);
        //    this.area.setZoomScale(1,true);
        //}
        runActionOfTipEffect();
        //点击提示音效;
    ...
    itemTouchFun:function(sender) {
        ...
        if (sender.texture) {
            //var cacheTextureForColor = cc.textureCache.getTextureColors(sender.texture);
        }
    ...
    //        sender.setLocalZOrder(100)
            ...
            if (curIdx == -1) {   //filter prize item
                ...
                //            var prizeStrArr = string.split2(prizeInfo,"`")
                //            roledata.updateRoleBaseData(tonumber(prizeStrArr[1]),tonumber(prizeStrArr[2]))
                ...
                if (this.game.itemAllCount != arr.length) {
                    ...
                    //                table.insert(searchItemLayer._curAndClickedItemList,{index = curIdx,name=itemName})
                    cc.log("insert table curAndClickedItemList ");
                    //bottom.setItemListPanelText(curIdx,itemName,item[CaseHubData.Iteminfo.ID])
                } else {
    ...
    //                refreshItemNum();  //更新文字放在找到物品，物品播放放大动作时;
        ...
        function pop() {
            ...
            if (this.game._input.taskId) {
                ...
                //var popup = new Popup(wuzheng,name,callbackFun);
                //popup.setPosition(vsize.width/2,vsize.height/2);
                //cc.director.getRunningScene().addChild(popup,10086);
                //popup.showPopup();
                //sender.setVisible(false);
                //this.game.stopGame(true);  //暂停整个找物品;
            }
        }
            ...
            if (sender.playIt) {
                ...
                sender.callback = function() {
                    if (sender.actionType == ITEM_TYPE.FIND_WITHOUT_ANIMATION) {   //找到 但是不往下飞的物品;
                        ...
                        pop();
                        deleteText();
                        //selfLayer.game._inventory:open();
                        //selfLayer.game._inventory:findItem(sender.target);
                    }
                }
                sender.playIt();
            } else {
